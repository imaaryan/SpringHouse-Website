...
export default function OtherPagesAdmin() {
  ...
  const fetchPages = async (page = 1) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/other-pages?page=${page}&limit=${pagination.limit}`,
      );
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(pages.map((p) => p._id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/admin/other-pages/${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleBulkDelete = async () => {
    if (
      !confirm(`Are you sure you want to delete ${selectedItems.length} pages?`)
    )
      return;
          ...
          {activeMenu === row._id && (
            <div className="absolute right-8 top-0 z-20 w-40 origin-top-right rounded-lg bg-white shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none py-1 border border-gray-100">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/admin/other-pages/edit/${row._id}`);
                }}
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Link href="/admin/other-pages/add">
