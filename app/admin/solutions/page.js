...
export default function SolutionsPage() {
  ...
  const fetchSolutions = async (page = 1) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/solutions?page=${page}&limit=${pagination.limit}`,
      );
  ...
  useEffect(() => {
    fetchSolutions(pagination.page);
  }, [pagination.page]);
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(solutions.map((p) => p._id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleBulkDelete = async () => {
    if (
      !confirm(
        `Are you sure you want to delete ${selectedItems.length} solutions?`,
      )
    )
      return;
    ...
    try {
      ...
      alert("Error deleting solutions");
    }
  };
          ...
          {activeMenu === row._id && (
            <div className="absolute right-8 top-0 z-20 w-40 origin-top-right rounded-lg bg-white shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none py-1 border border-gray-100">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/admin/solutions/edit/${row._id}`);
                }}
