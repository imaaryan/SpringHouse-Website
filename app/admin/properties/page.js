...
export default function PropertiesPage() {
  const router = useRouter(); // For edit navigation
  ...
  const fetchProperties = async (page = 1) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/properties?page=${page}&limit=${pagination.limit}`,
      );
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(properties.map((p) => p._id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleBulkDelete = async () => {
    if (
      !confirm(
        `Are you sure you want to delete ${selectedItems.length} properties?`,
      )
    )
      return;
    ...
    },
    {
      ...
      render: (row) => (
        <div className="text-sm text-gray-600">
          {row.area?.name || "Unknown Area"}, {row.city?.name || "Unknown City"}
        </div>
      ),
    },
    {
          ...
          {activeMenu === row._id && (
            <div className="absolute right-8 top-0 z-20 w-40 origin-top-right rounded-lg bg-white shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none py-1 border border-gray-100">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/admin/properties/edit/${row._id}`);
                }}
